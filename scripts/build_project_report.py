...
story += [
    ...
    bullet("Demonstrate five bounded agents without misrepresenting them as generative AI."),
...
story += [
    ...
    bullet("A successful order reduces browser stock; a decline keeps stock available for retry."),
    PageBreak(),
]
...
# Page 7 - frontend
story += page_title("06 / FRONTEND AND ORDER HISTORY", "Retail clarity with repeat-visit state", "Sunshine combines an original retail identity with a profile, seeded public order examples and private browser-persisted activity.")
story += [
    screenshot("07-profile-and-recent-orders.jpg", 150, 129),
    Spacer(1, 4 * mm),
    styled_table([
        ["Decision", "Reason"],
...
story += [
    ...
    bullet("The Notification Agent records the failure in the visitor's recent-order history."),
    Spacer(1, 4 * mm),
...
story += [
    ...
    bullet("The application has no authentication, shared order database or live stock source."),
